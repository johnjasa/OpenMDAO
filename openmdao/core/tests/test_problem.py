...
class TestProblem(unittest.TestCase):
    ...
    def test_feature_get_set_with_units_diff_err(self):
        ...
        try:
            ...
            self.assertEqual(str(err), "Group (<model>): The following inputs, ['C1.x', 'C2.x'], promoted to 'x', are connected but the metadata entries ['units', 'value'] differ. Call <group>.set_input_defaults('x', units=?, val=?), where <group> is the Group named '' to remove the ambiguity.")
        else:
